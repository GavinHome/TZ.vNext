<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
  ...
  <!-- created by Yves Lorphelin, largely inspired by the nunitsumary.xsl (see nantcontrib.sourceforge.net)-->
    ...
    <table width='90%'>
      <tr>
        <xsl:call-template name="ProduceTotalCoverageRate"/>
      </tr>
    </table>
  ...
  <xsl:template name="ProduceTotalCoverageRate">
    <td width="30%">
      <div class="assembly">
        <a>Total coverage:</a>
      </div>
    ...
    <td width="70%">
      <xsl:call-template name="detailPercent">
        <xsl:with-param name="visited" select="count(//seqpnt[not(@visitcount='0')] )" />
        <xsl:with-param name="notVisited" select="count(//seqpnt[ @visitcount='0' ][ @excluded='false' ] )" />
        <xsl:with-param name="total" select="count(//seqpnt[@excluded='false'])" />
      </xsl:call-template>
    </td>
  </xsl:template>
	<!-- General Header -->
